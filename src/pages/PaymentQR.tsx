  ...
  return (
    <PageLayout title="Payment QR">
      <div className="relative flex w-full flex-1 items-start justify-center p-4 pt-[80px]">
        <div className="relative isolate flex min-h-[350px] w-[289px] flex-col items-center gap-[30px] rounded-[16px] bg-white pb-[30px] pt-[60px]">
          {/* Avatar Circle */}
          ...
          <div className="mt-0 flex flex-col items-center gap-[10px]">
            <span className="font-outfit text-center text-[16px] font-medium leading-[20px] text-[#212121]">{user.name}</span>
            <span className="font-outfit mt-1 text-center text-[13px] font-normal leading-[16px] text-[#212121]">UPI ID - {user.upi}</span>
          </div>
          {/* QR Code */}
          <img src={user.qr} alt="QR Code" className="mt-[18px] h-[200px] w-[200px] rounded" />
        </div>
      </div>
    </PageLayout>
  );
